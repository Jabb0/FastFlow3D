        ...
        if features is not None:
            ...
            if self.use_xyz:
                if new_features is not None:
                    # TODO Check if this is correct (repeat new_features in the last dimension)
                    new_features = new_features.unsqueeze(-1).expand(-1, -1, -1, self.nsample)
                    new_features = torch.cat([grouped_xyz, grouped_features, new_features], dim=1)  # (B, C + 3, npoint, nsample)
                else:
