...
class LaserScanVis:
  ...
  def __init__(self, dataset, scan, scan_names, label_names, offset=0,
  ...
  def update_scan(self):
    ...
    # TODO here they do not iterate over the whole dataset... ask advisor
    # plot scan
    #mins = flows[:, :-1].min(axis=0)
    #maxs = flows[:, :-1].max(axis=0)

    rgb_flow = (flows[:, :-1] - self.mins) / (self.maxs - self.mins)
